...
<template>
  <view class="up">
    <view class="hd" v-if="us.isLoggedIn">
      <image :src="us.userInfo?.avatar||'/api/v1/files/default/avatar'" class="av"/>
      <view class="ifo"><text class="nm">{{us.userName}}</text><text class="rl">{{us.isStore?'店家':'用户'}}</text></view>
      <view v-if="us.isAdmin||us.isStore" class="hd-gear" @click="showAdmin=true">
        <uni-icons type="gear" size="24" color="#fff" />
      </view>
    </view>
    <view class="hd" v-else @click="goLogin">
      <view class="av"><uni-icons type="person" size="48" color="rgba(255,255,255,.8)" /></view>
      <view class="ifo"><text class="nm">点击登录</text></view>
    </view>
    <view class="ast">
    ...
    <view class="mu">
      <view class="mi" v-for="m in menu" :key="m.n" @click="goPage(m.p)">
        <text>{{m.n}}</text>
    ...
    <view class="admin-pop" v-if="showAdmin">
      <view class="admin-hd">
        <image :src="us.userInfo?.avatar||'/api/v1/files/default/avatar'" class="admin-av"/>
        <view><text class="admin-nm">{{us.userName}}</text><text class="admin-rl">{{us.isAdmin?'管理员':'店家'}}</text></view>
      ...
      <view class="admin-menu">
        ...
        <view class="admin-mi" v-if="us.isAdmin" @click="showAdmin=false;goPage('/pages/user/store-apply')">
          <view class="admin-mi-left">
...
.hd-gear{display:flex;align-items:center;justify-content:center;padding:8rpx;flex-shrink:0}
...
.admin-hd{display:flex;align-items:center;gap:16rpx;margin-bottom:24rpx}.admin-av{width:64rpx;height:64rpx;border-radius:50%;background:#eee}.admin-nm{font-size:30rpx;font-weight:600;display:block}.admin-rl{font-size:22rpx;color:#999}.admin-close{margin-left:auto}

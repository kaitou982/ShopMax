...
<template>
  <view class="cd" v-if="!loading && note">
    ...
    <view class="actions"><text :class="{on:note.isLiked}" @click="toggleLike">{{note.isLiked?'❤️':'🤍'}} {{note.likeCount}}</text><text :class="{on:note.isFavorited}" @click="toggleFav">{{note.isFavorited?'⭐':'🔖'}} {{note.favoriteCount}}</text><text>💬 {{note.commentCount}}</text></view>
...
.actions{display:flex;gap:40rpx;padding:16rpx 24rpx;border-top:1rpx solid #f0f0f0;text{font-size:28rpx}.on{color:#FF5000}}

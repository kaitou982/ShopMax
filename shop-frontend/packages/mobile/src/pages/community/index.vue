...
<template>
  ...
  <view class="cm-list"><view class="cm-card" v-for="n in notes" :key="n.id" @click="goDetail(n.id)">
    ...
    <view class="cm-ft"><text :class="{liked:n.isLiked}" @click.stop="toggleLike(n)">{{n.isLiked?'❤️':'🤍'}} {{n.likeCount}}</text><text>💬 {{n.commentCount}}</text></view>
  </view></view></view>
...
.cm-ft{display:flex;gap:32rpx;padding:12rpx 16rpx 20rpx;font-size:26rpx;text{cursor:pointer}.liked{animation:pop .3s ease}}@keyframes pop{0%,100%{transform:scale(1)}50%{transform:scale(1.2)}}
.fab{position:fixed;right:40rpx;bottom:140rpx;width:96rpx;height:96rpx;border-radius:50%;background:#000;display:flex;align-items:center;justify-content:center;box-shadow:0 8rpx 24rpx rgba(0,0,0,.2);z-index:100;text{font-size:48rpx;color:#fff;font-weight:300;line-height:1}}
</style>

...
<template>
  ...
  <template v-else-if="product">
    ...
    <view class="review-summary" v-if="reviewStats" @click="goReviews">
      <view class="rs-header"><text class="rs-title">商品评价</text><text class="rs-count">{{ reviewStats.totalCount }}条评价</text><text class="rs-arrow">›</text></view>
      <view class="rs-rate"><text class="rs-stars">{{ renderStars(Math.round(reviewStats.avgRating)) }}</text><text class="rs-avg">{{ reviewStats.avgRating?.toFixed(1) }}</text><text class="rs-good">好评率 {{ reviewStats.goodRate?.toFixed(0)||0 }}%</text></view>
      <view class="rs-preview" v-for="r in latestReviews" :key="r.id">
        <view class="rs-user"><text class="rs-name">{{ r.isAnonymous?'匿名':(r.userNickname||'用户') }}</text><text class="rs-ustars">{{ renderStars(r.rating) }}</text></view>
...
.rs-rate{display:flex;align-items:center;gap:12rpx;margin-bottom:16rpx}.rs-stars{font-size:24rpx;color:#FF9500}.rs-avg{font-size:28rpx;font-weight:700;color:#FF5000}.rs-good{font-size:22rpx;color:#999}

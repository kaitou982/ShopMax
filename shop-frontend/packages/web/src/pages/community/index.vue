  ...
  <div class="cm-page">
    ...
    <div class="cm-grid">
      <div class="cm-card" v-for="n in notes" :key="n.id" @click="goDetail(n.id)">
        ...
        <div class="cm-actions"><span :class="{ liked: n.isLiked }" @click="toggleLike(n)">{{ n.isLiked ? '❤️' : '🤍' }} {{ n.likeCount }}</span><span>💬 {{ n.commentCount }}</span></div>
      </div>
    </div>
  </div>
</template>
...
.cm-actions { display: flex; gap: 20px; padding: 10px 16px 14px; font-size: 13px; span { cursor: pointer; } .liked { animation: pop .3s ease; } }
